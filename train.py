    ...
    for epoch in range(max(0, last_epoch), args.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if y_g_hat is not None:
                ...
                # L1 Sample Loss
                min_dim = min(y.size(2), y_g_hat.size(2))
                y = y.narrow(2, 0, min_dim)
                y_g_hat = y_g_hat.narrow(2, 0, min_dim)
            ...
            if rank == 0:
                ...
                if steps % args.stdout_interval == 0:
                    with torch.no_grad():
                        min_dim = min(y_mel_loss.size(2), before_y_g_hat_mel.size(2))
                        y_mel_loss = y_mel_loss.narrow(2, 0, min_dim)
                        before_y_g_hat_mel = before_y_g_hat_mel.narrow(2, 0, min_dim)
                        mel_error = F.l1_loss(y_mel_loss, before_y_g_hat_mel).item()
                        
                        min_dim = min(y.size(2), before_y_g_hat.size(2))
                        y = y.narrow(2, 0, min_dim)
                        before_y_g_hat = before_y_g_hat.narrow(2, 0, min_dim)
...
def main():
    ...
    parser.add_argument('--train_file', default='training.txt')
    parser.add_argument('--valid_file', default='validation.txt')
